...
يلزم الحصول على مصدر الروم المستهدف المتزامن مع الإصدار المثبت وملفات القسم المطابقة لنفس البناء: `boot.img` أو `init_boot.img` عند وجوده، و`vendor_boot.img`، و`dtbo.img`، و`vendor_dlkm.img`. بعد ذلك يجب نقل Device Tree وتعريفات منصة SM7475، وتكييف أو إعادة بناء وحدات vendor، واستعادة BTF باستخدام سلسلة أدوات Android المتوافقة، ثم إنشاء مسار استرداد مجرّب قبل أي تفليش. راجع `ROM_COMPATIBILITY.md` لعقد التوافق العام مع الرومات.
